...
const TYPE_ICONS: Record<string, string> = {
  TV: "📺", Streaming: "🎬", Internet: "🌐", Musique: "🎵",
};

const JEU_ICONS: Record<string, string> = {
  societe: "🎲", console: "🎮", flechettes: "🎯",
};

type Tab = "tableau" | "stock" | "abonnements" | "jeux" | "budget";

export function FoyerBureau({ budget = 2000 }: { budget?: number }) {
  const [tab, setTab] = useState<Tab>("tableau");
  const [stock] = useState(DEMO_STOCK);
  const [abonnements] = useState(DEMO_ABONNEMENTS);
  const [jeux] = useState(DEMO_JEUX);
            ...
            <div className="rounded-[14px] bg-amber-50 p-3 dark:bg-amber-900/20">
              ...
              {alertes.map((a, i) => (
                <p key={i} className="text-[11px] text-amber-600">
                  {CAT_ICONS[a.categorie] || "📦"} {a.nom} — {a.qte} restant(s) (seuil : {a.seuil})
                </p>
              ))}
            </div>
          )}
        ...
        <div className="flex flex-col gap-2">
          {jeux.map((j, i) => (
            <div key={i} className="flex items-center justify-between rounded-[14px] bg-surface-elevated p-3 shadow-sm">
              <div className="flex items-center gap-3">
                <span className="text-2xl">{JEU_ICONS[j.type] || "🎲"}</span>
                <div>
