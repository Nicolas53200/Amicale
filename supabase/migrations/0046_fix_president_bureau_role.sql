-- Migration 0046: Fix bureau_role value for presidents
-- The setup_organization function sets bureau_role to 'Président' (French label)
-- but the app expects 'president' (lowercase slug). Fix existing data and the function.

-- Fix existing presidents
UPDATE members
SET bureau_role = 'president'
WHERE bureau_role = 'Président';

-- Recreate the setup function with the correct value
