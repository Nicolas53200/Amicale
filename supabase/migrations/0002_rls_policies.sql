...
-- auth.org_id() extracts the org_id from the JWT custom claim.
...
CREATE POLICY "commission_members_select" ON commission_members
  FOR SELECT USING (
    EXISTS (
      ...
      AND commissions.org_id = auth.org_id()
    )
  );
